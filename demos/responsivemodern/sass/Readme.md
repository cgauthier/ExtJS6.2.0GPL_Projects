...
folder are mapped to the application's root namespace, 'ResponsiveModern'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ResponsiveModern"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ResponsiveModern' in the same way
as `"ResponsiveModern/sass/src"`.
